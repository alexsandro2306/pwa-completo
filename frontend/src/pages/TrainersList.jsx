...
const TrainersList = () => {
    ...
    const handleSendRequest = async (trainerId) => {
        // ✅ VALIDAÇÃO: Verificar se cliente já tem trainer
        if (user?.trainer) {
    ...
    const openRequestModal = (trainer) => {
        // ✅ VALIDAÇÃO: Bloquear se já tiver trainer
        if (user?.trainer) {
                    ...
                    {trainers.map((trainer) => (
                        <div key={trainer._id} className="trainer-card glass">
                            ...
                            <div className="trainer-stats">
                                <div className="stat">
                                    <Users size={18} />
                                    <span>{trainer.clients?.length || 0} clientes</span>
                                </div>
                                <div className="stat">
                                    <Award size={18} />
                            ...
                            <div className="trainer-actions">
                                ...
                                <button
                                    className={`btn-primary ${user?.trainer ? 'disabled' : ''}`}
                                    onClick={() => openRequestModal(trainer)}
                                    disabled={user?.trainer}
                                >
                                    <Send size={18} />
                                    {user?.trainer ? 'Já tens Trainer' : 'Enviar Pedido'}
                                </button>
                            </div>
                        </div>
                    ))}
