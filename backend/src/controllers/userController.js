...
exports.getValidatedTrainers = async (req, res) => {
    try {
        const trainers = await User.find({
            ...
            .select('firstName lastName email avatar bio clients')
            .populate('clients', 'firstName')
            .lean();

        // Adicionar contagem de clientes para cada trainer
        const trainersWithCount = trainers.map(trainer => ({
            ...trainer,
            clientCount: trainer.clients?.length || 0,
            clients: undefined // Remover array de clientes dos dados retornados
        }));
