        ...
        const fetchHomeData = async () => {
            try {
                if (user.role === 'client') {
                    ...
                    // Fetch recent logs for all clients to show in dashboard
                    // For now, we simulate/approximate or fetch if endpoint exists
                    // Let's assume we want to show the trainer some "Insights"
                    setTrainerInsights({
                        activeCount: clients.length,
                        unreadMsgs: unreadRes.data.data?.length || 0,
                        recentLogs: [], // This would ideally be a dedicated endpoint
                        expiringPlans: []
                    });
        ...
        <div className="home-container animate-fade">
            ...
            <div className="stats-grid">
                {user.role === 'client' ? (
                    ...
                    <>
                        <StatCard icon={UserCheck} label="Trainers Pendentes" value={adminStats.pendingTrainers} color="#f59e0b" onClick={() => navigate('/trainers')} />
                        <StatCard icon={ClipboardList} label="Pedidos de Mudança" value={adminStats.pendingRequests} color="#3b82f6" onClick={() => navigate('/requests')} />
                        <StatCard icon={Users} label="Total Trainers" value={adminStats.totalTrainers} color="#10b981" onClick={() => navigate('/trainers')} />
                    </>
                )}
            </div>
            ...
            <style jsx>{`
                ...
                @media (max-width: 1000px) { .trainer-dashboard-grid { grid-template-columns: 1fr; } }
