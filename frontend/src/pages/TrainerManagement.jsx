...
const TrainerManagement = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // For now, listing all users with role trainer that are not in pending
            // You might need a specific endpoint for 'all active trainers' if available
            // For now, listing all users with role trainer that are not in pending
            // You might need a specific endpoint for 'all active trainers' if available
            const allUsersRes = await api.get('/users/trainers');
            setActiveTrainers(allUsersRes.data.data);
        } catch (err) {
    ...
    return (
            ...
            <section className="mgmt-section">
                ...
                <div className="trainer-grid">
                    {pendingTrainers.length === 0 ? (
                        <p className="empty-msg">Nenhum treinador pendente no momento.</p>
                    ) : (
                        pendingTrainers.map(trainer => (
                            <div key={trainer._id} className="glass trainer-card pending">
                                <div className="trainer-info">
                                    <div className="avatar">
                                        {trainer.avatar ? <img src={trainer.avatar} alt={trainer.firstName} /> : <User size={24} />}
                                    </div>
                                    <div>
        ...
        .trainer-table-container { overflow-x: auto; padding: 0.5rem; }
