...
const ClientStatsDashboard = () => {
    ...
    const isMe = !clientId; // If no clientId, it's the client viewing their own stats
    ...
    const fetchData = async () => {
        try {
            ...
            if (isMe) {
                // Fetching my own data
                const [uRes, sRes, lRes, pRes] = await Promise.all([
                    api.get('/users/me'),
                    api.get('/users/dashboard/me'),
                    api.get('/workouts/client/logs'),
                    api.get('/workouts', { params: { clientId: user._id } }) // Assuming creating endpoint handles filtering if passed or auto-filters
                ]);
                clientData = uRes.data.user; // /users/me returns { user: ... }
                statsData = sRes.data.data;
                logsData = lRes.data.data;
                plansData = pRes.data.data || []; // check response structure
            } else {
                ...
                clientData = cRes.data.data; // /users/:id returns { data: ... }
                statsData = sRes.data.data;
                logsData = lRes.data.data;
            ...
            setPlans(plansData ? (Array.isArray(plansData) ? plansData : plansData.plans || []) : []); // Safety check
        } catch (err) {
