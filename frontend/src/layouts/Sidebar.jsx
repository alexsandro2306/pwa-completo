...
const Sidebar = () => {
    ...
    const menuItems = [
        // Admin Menu - SEM "Início" duplicado
        { icon: <Home size={20} />, label: 'Dashboard', path: '/admin', roles: ['admin'], end: true }, // ← end: true
        ...
        // Shared (Trainer + Client only)
        {
            ...
            end: false // ← Chat pode ter sub-rotas como /chat/:userId
        },
            ...
            <nav>
                {filteredMenu.map((item) => (
                    <NavLink
                        key={item.path}
                        to={item.path}
                        end={item.end} // ← IMPORTANTE: prop 'end' para match exato
